...
namespace escortagram.Common
{
    public class CommonFunction
    {
        ...
        public void SaveFile(HttpPostedFileBase file,string Guid , string Type)
        {
            ...
            {
                string savePathP = Path.Combine(System.Web.HttpContext.Current.Server.MapPath(@"\\Upload\\ModelImages\\"));
                savePathP += Guid + ".jpg";
                if (File.Exists(savePathP))
                {
                    File.Delete(savePathP);
                    file.SaveAs(savePathP);
                }
                else
                {
                    file.SaveAs(savePathP);
